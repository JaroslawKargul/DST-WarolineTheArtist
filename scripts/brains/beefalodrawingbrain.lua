...
local BeefaloDrawingBrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
	self.name = "beefalodrawingbrain"
end)
...
function BeefaloDrawingBrain:OnStart()
    local root = PriorityNode(
    {
		...
		RunAway(self.inst, ShouldRunAway, SEE_MONSTER_DIST, SAFE_MONSTER_DIST), -- avoid anything that could kill me
        ...
        Wander(self.inst, function() return self.inst.components.knownlocations:GetLocation("home") end, MAX_WANDER_DIST, {minwalktime=.6, randwalktime=.2, minwaittime=7, randwaittime=10}),
    }, .25)
    self.bt = BT(self.inst, root)
end

return BeefaloDrawingBrain
