...
local SmallBirdDrawingBrain = Class(Brain, function(self, inst)
    Brain._ctor(self, inst)
	self.name = "smallbirddrawingbrain"
end)
...
function SmallBirdDrawingBrain:OnStart()
    local root =
    PriorityNode({
        ...
        Wander(self.inst, function() return self.inst.components.knownlocations:GetLocation("home") end, MAX_WANDER_DIST, {minwalktime=.5, randwalktime=.5, minwaittime=6, randwaittime=3}),
    },.25)
    self.bt = BT(self.inst, root)
 end

return SmallBirdDrawingBrain